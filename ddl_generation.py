...
check_list = ['B_CUSTOMER_MERGE_CHAIN']
...
HEADER = '''
/*
 * NOTES: Creates [RAPLACE_TABLE_NM] for AspiraOne datamart 
 *
...
INDEX = '''
IF NOT EXISTS( SELECT TOP 1 1 FROM sys.indexes i WHERE i.object_id = object_id('[dbo].[[RAPLACE_TABLE_NM]]','U') AND i.name = '[RAPLACE_TABLE_NM]_[RAPLACE_COLUMN_NM]_IX')
BEGIN
    CREATE NONCLUSTERED INDEX [[RAPLACE_TABLE_NM]_[RAPLACE_COLUMN_NM]_IX] ON [dbo].[[RAPLACE_TABLE_NM]]([[RAPLACE_COLUMN_NM]]) 
    ...
    ON {INDEXFG}
    PRINT '[INFO] CREATED NONCLUSTERED INDEX [dbo].[[RAPLACE_TABLE_NM]].[[RAPLACE_TABLE_NM]_[RAPLACE_COLUMN_NM]_IX]'
END
'''
